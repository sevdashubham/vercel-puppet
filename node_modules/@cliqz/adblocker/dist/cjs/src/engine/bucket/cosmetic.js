...
function createLookupTokens(hostname, domain) {
    const hostnamesHashes = request_1.getHostnameHashesFromLabelsBackward(hostname, domain);
    const entitiesHashes = request_1.getEntityHashesFromLabelsBackward(hostname, domain);
...
class CosmeticFilterBucket {
    ...
    getBaseRules = true, getInjectionRules = true, getExtendedRules = true, getRulesFromDOM = true, getRulesFromHostname = true, }) {
        ...
        if (allowGenericHides === true && getRulesFromDOM === true && classes.length !== 0) {
            this.classesIndex.iterMatchingFilters(utils_1.hashStrings(classes), (rule) => {
                if (rule.match(hostname, domain)) {
                    rules.push(rule);
                }
                return true;
        ...
        if (allowGenericHides === true && getRulesFromDOM === true && ids.length !== 0) {
            this.idsIndex.iterMatchingFilters(utils_1.hashStrings(ids), (rule) => {
                if (rule.match(hostname, domain)) {
                    rules.push(rule);
                }
                return true;
        ...
        if (allowGenericHides === true && getRulesFromDOM === true && hrefs.length !== 0) {
            this.hrefsIndex.iterMatchingFilters(compact_set_1.compactTokens(compact_set_1.concatTypedArrays(hrefs.map((href) => utils_1.tokenizeNoSkip(href)))), (rule) => {
                if (rule.match(hostname, domain)) {
                    rules.push(rule);
                }
                return true;
