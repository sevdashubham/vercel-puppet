...
const HTTP_HASH = utils_1.fastHash('http');
const HTTPS_HASH = utils_1.fastHash('https');
function isAllowedHostname(ch) {
    return (utils_1.isDigit(ch) || utils_1.isAlpha(ch) || ch === 95 /* '_' */ || ch === 45 /* '-' */ || ch === 46 /* '.' */);
}
...
const REQUEST_TYPE_TO_MASK = {
    ...
    cspviolationreport: 128 /* fromOther */,
    ...
    other: 128 /* fromOther */,
    preflight: 128 /* fromOther */,
    signedexchange: 128 /* fromOther */,
...
class NetworkFilter {
    ...
    static parse(line, debug = false) {
        ...
        if (line.charCodeAt(0) === 64 /* '@' */ && line.charCodeAt(1) === 64 /* '@' */) {
            filterIndexStart += 2;
            mask = utils_1.setBit(mask, 134217728 /* isException */);
        }
        ...
        if (optionsIndex !== -1 && line.charCodeAt(optionsIndex + 1) !== 47 /* '/' */) {
            ...
            for (const rawOption of line.slice(optionsIndex + 1).split(',')) {
                ...
                switch (option) {
                    ...
                    case 'badfilter':
                        mask = utils_1.setBit(mask, 131072 /* isBadFilter */);
                        break;
                    case 'important':
                        ...
                        mask = utils_1.setBit(mask, 1048576 /* isImportant */);
                        break;
                    case 'match-case':
                        // Note: `negation` should always be `false` here.
                        if (negation) {
                    ...
                    case 'third-party':
                        if (negation) {
                            ...
                            mask = utils_1.clearBit(mask, 32768 /* thirdParty */);
                        }
                        else {
                            // third-party means ~first-party
                            mask = utils_1.clearBit(mask, 16384 /* firstParty */);
                        }
                        break;
                    case '1p':
                    case 'first-party':
                    ...
                    case 'redirect':
                        ...
                        if (option === 'redirect-rule') {
                            mask = utils_1.setBit(mask, 536870912 /* isRedirectRule */);
                        }
                    ...
                    case 'csp':
                        ...
                        mask = utils_1.setBit(mask, 262144 /* isCSP */);
                        if (optionValue.length > 0) {
                            csp = optionValue;
                        }
                        break;
                    ...
                    case 'elemhide':
                        ...
                        mask = utils_1.setBit(mask, 524288 /* isGenericHide */);
                        mask = utils_1.setBit(mask, 2097152 /* isSpecificHide */);
                        break;
                    case 'shide':
                    case 'specifichide':
                        if (negation) {
                    ...
                    default: {
                        ...
                        if (negation) {
                            cptMaskNegative = utils_1.clearBit(cptMaskNegative, optionMask);
                        }
                        else {
                            cptMaskPositive = utils_1.setBit(cptMaskPositive, optionMask);
                        }
                        break;
                    }
                }
        ...
        if (filterIndexEnd - filterIndexStart >= 2 &&
            ...
            mask = utils_1.setBit(mask, 4194304 /* isFullRegex */);
        }
        else {
            // Deal with hostname pattern
            if (filterIndexEnd > 0 && line.charCodeAt(filterIndexEnd - 1) === 124 /* '|' */) {
                mask = utils_1.setBit(mask, 67108864 /* isRightAnchor */);
                filterIndexEnd -= 1;
            }
            if (filterIndexStart < filterIndexEnd &&
                ...
                if (filterIndexStart < filterIndexEnd - 1 &&
                    ...
                    mask = utils_1.setBit(mask, 268435456 /* isHostnameAnchor */);
                    filterIndexStart += 2;
                }
                else {
                    mask = utils_1.setBit(mask, 33554432 /* isLeftAnchor */);
                    filterIndexStart += 1;
                }
            }
            ...
            if (utils_1.getBit(mask, 268435456 /* isHostnameAnchor */)) {
                ...
                if (firstSeparator === filterIndexEnd) {
                    ...
                    if (separatorCode === 94 /* '^' */) {
                        ...
                        if (filterIndexEnd - filterIndexStart === 1) {
                            ...
                            mask = utils_1.setBit(mask, 8388608 /* isRegex */);
                            mask = utils_1.setBit(mask, 33554432 /* isLeftAnchor */);
                        }
                    }
            ...
            if (utils_1.getBit(mask, 268435456 /* isHostnameAnchor */) === false &&
                filterIndexEnd - filterIndexStart > 0 &&
                line.charCodeAt(filterIndexStart) === 42 /* '*' */) {
                mask = utils_1.clearBit(mask, 33554432 /* isLeftAnchor */);
                filterIndexStart += 1;
            }
            // Transform filters on protocol (http, https, ws)
            if (utils_1.getBit(mask, 33554432 /* isLeftAnchor */)) {
                if (filterIndexEnd - filterIndexStart === 5 &&
                    utils_1.fastStartsWithFrom(line, 'ws://', filterIndexStart)) {
                    mask = utils_1.setBit(mask, 4096 /* fromWebsocket */);
                    mask = utils_1.clearBit(mask, 33554432 /* isLeftAnchor */);
                    mask = utils_1.clearBit(mask, 4 /* fromHttp */);
                    mask = utils_1.clearBit(mask, 8 /* fromHttps */);
                    filterIndexStart = filterIndexEnd;
                }
                else if (filterIndexEnd - filterIndexStart === 7 &&
                    utils_1.fastStartsWithFrom(line, 'http://', filterIndexStart)) {
                    mask = utils_1.setBit(mask, 4 /* fromHttp */);
                    mask = utils_1.clearBit(mask, 8 /* fromHttps */);
                    mask = utils_1.clearBit(mask, 33554432 /* isLeftAnchor */);
                    filterIndexStart = filterIndexEnd;
                }
                else if (filterIndexEnd - filterIndexStart === 8 &&
                    utils_1.fastStartsWithFrom(line, 'https://', filterIndexStart)) {
                    mask = utils_1.setBit(mask, 8 /* fromHttps */);
                    mask = utils_1.clearBit(mask, 4 /* fromHttp */);
                    mask = utils_1.clearBit(mask, 33554432 /* isLeftAnchor */);
                    filterIndexStart = filterIndexEnd;
                }
            ...
            if (filterIndexEnd - filterIndexStart > 0) {
                ...
                mask = setNetworkMask(mask, 16777216 /* isUnicode */, utils_1.hasUnicode(filter));
                if (utils_1.getBit(mask, 8388608 /* isRegex */) === false) {
    ...
    static deserialize(buffer) {
        ...
        const isUnicode = utils_1.getBit(mask, 16777216 /* isUnicode */);
    ...
    getSerializedSize(compression) {
        ...
        if (this.csp !== undefined) {
            estimate += data_view_1.sizeOfNetworkCSP(this.csp, compression);
        }
        if (this.filter !== undefined) {
            if (this.isUnicode() === true) {
                estimate += data_view_1.sizeOfUTF8(this.filter);
            }
            else {
                estimate += data_view_1.sizeOfNetworkFilter(this.filter, compression);
            }
        }
        if (this.hostname !== undefined) {
            estimate += data_view_1.sizeOfNetworkHostname(this.hostname, compression);
        }
        if (this.domains !== undefined) {
            estimate += this.domains.getSerializedSize();
        }
        if (this.rawLine !== undefined) {
            estimate += data_view_1.sizeOfRawNetwork(this.rawLine, compression);
        }
        if (this.redirect !== undefined) {
            estimate += data_view_1.sizeOfNetworkRedirect(this.redirect, compression);
        }
        if (this.denyallow !== undefined) {
    ...
    toString() {
        ...
        if (this.fromAny() === false) {
            const numberOfCptOptions = utils_1.bitCount(this.getCptMask());
            const numberOfNegatedOptions = utils_1.bitCount(FROM_ANY) - numberOfCptOptions;
    ...
    isRedirectRule() {
        return utils_1.getBit(this.mask, 536870912 /* isRedirectRule */);
    }
    getRedirect() {
    ...
    getTokens() {
        ...
        if (this.isFullRegex() === false) {
            if (this.filter !== undefined) {
                ...
                utils_1.tokenizeWithWildcardsInPlace(this.filter, skipFirstToken, skipLastToken, tokens_buffer_1.TOKENS_BUFFER);
            }
            // Append tokens from hostname, if any
            if (this.hostname !== undefined) {
                utils_1.tokenizeInPlace(this.hostname, false, this.filter !== undefined && this.filter.charCodeAt(0) === 42 /* '*' */, tokens_buffer_1.TOKENS_BUFFER);
            }
        }
        else if (this.filter !== undefined) {
            utils_1.tokenizeRegexInPlace(this.filter, tokens_buffer_1.TOKENS_BUFFER);
        }
    ...
    isCptAllowed(cpt) {
        ...
        if (mask !== undefined) {
            return utils_1.getBit(this.mask, mask);
        }
    ...
    isException() {
        return utils_1.getBit(this.mask, 134217728 /* isException */);
    }
    isHostnameAnchor() {
        return utils_1.getBit(this.mask, 268435456 /* isHostnameAnchor */);
    }
    isRightAnchor() {
        return utils_1.getBit(this.mask, 67108864 /* isRightAnchor */);
    }
    isLeftAnchor() {
        return utils_1.getBit(this.mask, 33554432 /* isLeftAnchor */);
    }
    isImportant() {
        return utils_1.getBit(this.mask, 1048576 /* isImportant */);
    }
    isFullRegex() {
        return utils_1.getBit(this.mask, 4194304 /* isFullRegex */);
    }
    isRegex() {
        return (utils_1.getBit(this.mask, 8388608 /* isRegex */) ||
            utils_1.getBit(this.mask, 4194304 /* isFullRegex */));
    }
    isPlain() {
        return !this.isRegex();
    }
    isCSP() {
        return utils_1.getBit(this.mask, 262144 /* isCSP */);
    }
    isElemHide() {
        return this.isSpecificHide() && this.isGenericHide();
    }
    isSpecificHide() {
        return utils_1.getBit(this.mask, 2097152 /* isSpecificHide */);
    }
    isGenericHide() {
        return utils_1.getBit(this.mask, 524288 /* isGenericHide */);
    }
    isBadFilter() {
        return utils_1.getBit(this.mask, 131072 /* isBadFilter */);
    }
    isUnicode() {
        return utils_1.getBit(this.mask, 16777216 /* isUnicode */);
    }
    fromAny() {
        return this.getCptMask() === FROM_ANY;
    }
    thirdParty() {
        return utils_1.getBit(this.mask, 32768 /* thirdParty */);
    }
    firstParty() {
        return utils_1.getBit(this.mask, 16384 /* firstParty */);
    }
    fromImage() {
        return utils_1.getBit(this.mask, 16 /* fromImage */);
    }
    fromMedia() {
        return utils_1.getBit(this.mask, 32 /* fromMedia */);
    }
    fromObject() {
        return utils_1.getBit(this.mask, 64 /* fromObject */);
    }
    fromOther() {
        return utils_1.getBit(this.mask, 128 /* fromOther */);
    }
    fromPing() {
        return utils_1.getBit(this.mask, 256 /* fromPing */);
    }
    fromScript() {
        return utils_1.getBit(this.mask, 512 /* fromScript */);
    }
    fromStylesheet() {
        return utils_1.getBit(this.mask, 1024 /* fromStylesheet */);
    }
    fromDocument() {
        return utils_1.getBit(this.mask, 1 /* fromDocument */);
    }
    fromSubdocument() {
        return utils_1.getBit(this.mask, 2048 /* fromSubdocument */);
    }
    fromWebsocket() {
        return utils_1.getBit(this.mask, 4096 /* fromWebsocket */);
    }
    fromHttp() {
        return utils_1.getBit(this.mask, 4 /* fromHttp */);
    }
    fromHttps() {
        return utils_1.getBit(this.mask, 8 /* fromHttps */);
    }
    fromXmlHttpRequest() {
        return utils_1.getBit(this.mask, 8192 /* fromXmlHttpRequest */);
    }
    fromFont() {
        return utils_1.getBit(this.mask, 2 /* fromFont */);
    }
}
...
function setNetworkMask(mask, m, value) {
    if (value === true) {
        return utils_1.setBit(mask, m);
    }
    return utils_1.clearBit(mask, m);
}
/**
...
function checkPattern(filter, request) {
    ...
    if (filter.isHostnameAnchor() === true) {
        ...
        if (filter.isRegex()) {
            ...
            // with nothing in between. So we extract the part of the URL following
            // after hostname and will perform the matching on it.
            return utils_1.fastStartsWithFrom(request.url, pattern, request.url.indexOf(filterHostname) + filterHostname.length);
        }
        if (filter.hasFilter() === false) {
            return true;
        }
        ...
        // |pattern
        return utils_1.fastStartsWith(request.url, pattern);
    }
    else if (filter.isRightAnchor()) {
        // pattern|
        return request.url.endsWith(pattern);
